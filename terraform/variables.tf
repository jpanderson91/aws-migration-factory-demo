...
variable "project_name" {
  ...
  type        = string
  default     = "aws-migration-factory-demo"
...
variable "{{PRIMARY_SERVICE_VAR}}" {
  description = "Configuration for Kinesis"
  type = object({
    enabled        = bool
    {{SERVICE_CONFIG_VARS}}
  })
...
variable "storage_config" {
  ...
  default = {
    bucket_prefix    = "aws-migration-factory-demo"
    versioning      = true
    encryption      = true
    lifecycle_days  = 30
  }
...
variable "monitoring_config" {
  ...
  default = {
    ...
    metric_namespace    = "aws-migration-factory-demo/Kinesis"
  }
}
